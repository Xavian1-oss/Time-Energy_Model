...
class Exp_Main(Exp_Basic):
    def __init__(self, args):
        ...
        clear_ml_test_mode = (not self.args.should_log)
        clear_ml_wrapper = SFDB_logger(
            args=parsed_args,
            project_name=self.args.name,
            test_mode=clear_ml_test_mode,
        )
        self.logger = clear_ml_wrapper.get_logger()

    def _model_checkpoint_path(self, setting) -> str:
        
        model_checkpoint_path = os.path.join(self.args.checkpoints + setting, 'checkpoint.pth')
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.logger.report_scalar(title=f'Training loss',
                                      series='MSE',
            ...
            self.logger.report_scalar(title=f'Validation loss',
                                      series='MSE',
                                      value=float(vali_loss),
                                      iteration=epoch + 1)

            self.logger.report_scalar(title=f'Testing loss',
                                      series='MSE',
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(self._model_checkpoint_path(setting)))
